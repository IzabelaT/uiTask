...
function Task(props) {
  ...
  var task_loaded = useSelector(
    (state) => state["general"].task[props.title]
  );
  var task_synced = useSelector((state) => state["general"].sync[props.title]);
  ...
  useEffect(async () => {
    if (!task_loaded) {
      ...
      dispatch(changeTaskLoaded({ list: props.title, value: 1 }));
    } else {
      // this body is executed each time the page re-renders (after being task_loaded) => updates 'cache' at each render
      localStorage.setItem(
        props.title,
        JSON.stringify({ tasks: tasks, time: time })
      );
      /* auto sync */
      if (signedIn && task_loaded && fileId !== 0 && upload && !isUploading) {
    ...
    if (signedIn && task_loaded && !task_synced && fileId !== 0) {
      dispatch(changeTaskSyncStat({ list: props.title, value: 1 })); // set the sync status for this list of tasks as done
            ...
            <Col xs={24} sm={12} md={8} lg={8}>
              ...
              >
                ...
                <Nav.Item eventKey="completed">
                  <span className="taskTabs">Completed</span>
                </Nav.Item>
              </Nav>
            </Col>
          </Row>
